...
export default function WatchPageClient({ movie, posterUrl, tmdbData, seasonData, tmdbCollectionData }: WatchPageClientProps) {
  ...
  const getExtendedEpisodes = () => {
    ...
    if (tmdbId || imdbId) {
      ...
      if (tmdbId) {
        ...
        // Add VidLink Anime server for anime content
        if (movie.type === 'hoathinh' || (movie.category && movie.category.some((c: any) => c.name?.toLowerCase().includes('anime')))) {
          baseEpisodes.push({
            server_name: "VidLink Anime (MAL)",
            server_data: generateServerData((epNum) => {
              const baseUrl = isTVShow 
                ? `https://vidlink.pro/anime/{mal_id}/${epNum}/sub?fallback=true` 
                : `https://vidlink.pro/anime/{mal_id}/1/sub?fallback=true`;
              // Note: MAL ID needs to be fetched dynamically via Jikan API
              return baseUrl;
            })
          });
        }
    ...
    if (isExternalSource && currentEpisode && imdbId) {
      ...
      fetch(fetchUrl)
        ...
        .then(data => {
           ...
           // Fetch thêm từ OpenSubtitles API
           let osUrl = `/api/opensubtitles?imdbId=${imdbId}`;
           if (movie.type === 'series' || movie.type === 'hoathinh') {
             osUrl += `&season=${seasonNum}&episode=${epNum}`;
           }

           fetch(osUrl)
            .then(res => res.json())
            .then(osData => {
               if (osData.subtitles && osData.subtitles.length > 0) {
                  newSubs.push(...osData.subtitles);
               }
  ...
  useEffect(() => {
    const handleVidlinkMessage = (event: MessageEvent) => {
      ...
      // Handle MEDIA_DATA for watch progress tracking
      if (event.data?.type === 'MEDIA_DATA') {
        const mediaData = event.data.data;
        try {
          localStorage.setItem('vidLinkProgress', JSON.stringify(mediaData));
        } catch (e) {
          console.error('Error saving VidLink progress:', e);
        }
      }
      
      // Handle PLAYER_EVENT for real-time tracking
  ...
  useEffect(() => {
    if (currentServerName === "VidLink Anime (MAL)") {
      ...
      fetch(`https://api.jikan.moe/v4/anime?q=${encodeURIComponent(searchTitle)}&limit=1`)
        ...
        .then(data => {
            if (data.data && data.data.length > 0) {
               ...
               const match = currentEpisode?.name?.match(/\d+/);
               if (match) epNum = parseInt(match[0], 10);
               setMalAnimeUrl(`https://vidlink.pro/anime/${malId}/${epNum}/sub`);
            }
        })
        .catch(console.error);
    }
  ...
  if (currentServerName === "VidLink Anime (MAL)" && malAnimeUrl) {
    finalEmbedUrl = malAnimeUrl;
  }
  
  if (finalEmbedUrl) {
    // Add startAt parameter for VidLink using saved progress
    if (currentServerName?.toLowerCase().includes("vidlink") && typeof window !== "undefined") {
      const savedProgress = localStorage.getItem(`playback_progress_${movie.slug}_ep_${currentEpisodeIndex}`);
      if (savedProgress) {
        const startTime = parseFloat(savedProgress);
        if (startTime > 10) { // Only add startAt if progress > 10 seconds
          const separator = finalEmbedUrl.includes('?') ? '&' : '?';
          finalEmbedUrl += `${separator}startAt=${Math.floor(startTime)}`;
        }
      }
      
      // Add external subtitles for VidLink
      if (subtitlesData.length > 0) {
        const separator = finalEmbedUrl.includes('?') ? '&' : '?';
        const firstSub = subtitlesData[0];
        finalEmbedUrl += `${separator}sub_file=${encodeURIComponent(firstSub.file)}&sub_label=${encodeURIComponent(firstSub.label)}`;
      }
    }
  }
