...
const MovieSlider = memo(function MovieSlider({ movies }: MovieSliderProps) {
  const { ref: viewRef, inView } = useInView({
    triggerOnce: true,
    rootMargin: "1500px 0px", // Preload when 1500px away from viewport to prevent flash
  });
  ...
  return (
    <div 
      ref={viewRef} 
      className="relative group/slider min-h-[300px]"
    >
      {!inView ? (
        <div className="flex overflow-hidden pb-6 pt-6 -mx-2 px-2" style={{ marginLeft: '-1rem' }}>
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="flex-[0_0_50%] md:flex-[0_0_25%] lg:flex-[0_0_16.6666%] min-w-0 pl-4">
              <div className="w-full aspect-[2/3] bg-zinc-800/30 animate-pulse rounded-xl border border-white/5"></div>
            </div>
          ))}
        </div>
      ) : (
