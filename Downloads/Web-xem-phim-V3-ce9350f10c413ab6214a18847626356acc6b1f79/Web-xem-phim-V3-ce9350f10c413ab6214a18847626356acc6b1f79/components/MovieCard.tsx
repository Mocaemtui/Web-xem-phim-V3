...
const MovieCard = memo(function MovieCard({ movie, posterUrl, href, isHistory, priority = false }: MovieCardProps) {
  ...
  // Màu nền giữ cố định, không thay đổi khi ảnh tải để tránh chớp đen
      ...
      hoverTimerRef.current = setTimeout(async () => {
        try {
          ...
          if (isMobile) {
            ...
            const tUrl = data.movie?.thumb_url || data?.data?.item?.thumb_url;
            if (tUrl) {
              setThumbImageUrl(resolveImgUrl(tUrl));
            }
          } else {
            const trUrl = data.movie?.trailer_url || data?.data?.item?.trailer_url;
            const tUrl = data.movie?.thumb_url || data?.data?.item?.thumb_url;
            if (trUrl) {
              const embed = getYoutubeEmbedUrl(trUrl);
              if (embed) {
                setTrailerUrl(embed);
              } else if (tUrl) {
                setThumbImageUrl(resolveImgUrl(tUrl));
  ...
  return (
    <div className="h-full z-10 hover:z-30 relative transition-transform duration-300 ease-out hover:scale-105">
      ...
      >
      <div 
        className="relative aspect-[2/3] overflow-hidden rounded-xl shadow-lg border border-white/5 bg-zinc-800/70"
        style={{ 
      ...
      >
        ...
        <Image
          ...
          className={`object-cover transition-opacity duration-500 ease-in group-hover:scale-110 ${trailerUrl ? 'opacity-0' : 'opacity-100'}`}
          sizes="(max-width: 768px) 50vw, (max-width: 1200px) 33vw, 16vw"
          loading={priority ? "eager" : "lazy"}
          priority={priority}
          quality={60}
          placeholder="blur"
          blurDataURL="data:image/jpeg;base64,"
        />
