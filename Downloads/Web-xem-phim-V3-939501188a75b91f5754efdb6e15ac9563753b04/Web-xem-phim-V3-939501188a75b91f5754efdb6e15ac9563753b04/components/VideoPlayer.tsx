...
export default function VideoPlayer({
  ...
  const applyVolume = useCallback((vol: number) => {
    ...
    // On iOS, volume is controlled by hardware buttons, not programmatically
    if (isIOS) {
      setVolume(vol); // Update state but don't set video.volume
      return;
    }
  ...
  useEffect(() => {
    ...
    // iOS Safari has native HLS support - use it instead of hls.js
    if (isIOS && video.canPlayType("application/vnd.apple.mpegurl")) {
      video.src = videoUrl;
      // For iOS, let the native player handle loading and user interaction
      // Don't force autoplay to avoid blocking the media element
      // Add event listener for when video is ready to play
      handleCanPlay = () => {
        if (handleCanPlay) {
          video.removeEventListener('canplay', handleCanPlay);
        }
        // For iOS, attempt muted autoplay first, then unmute on user interaction
    ...
    } else if (Hls.isSupported() && (videoUrl.includes(".m3u8") || videoUrl.includes("m3u8"))) {
      hls = new Hls({
        enableWorker: true,
        lowLatencyMode: true,
        backBufferLength: 90,
  ...
  }, [videoRef, onBuffering, onPlaySync, onPauseSync, onSeekSync, isWatchTogether, resetControlsTimer, isHost]);
  ...
  const toggleFullscreen = () => {
    ...
    if (isMobile || isIOS) {
      // On mobile/iOS, request native fullscreen directly on the video element
  ...
  // Keyboard Hotkeys
  useEffect(() => {
    // Skip keyboard hotkeys on iOS (touch devices)
    if (isIOS) return;
  ...
  }, [videoRef, togglePlay, toggleFullscreen, onSeekSync, isIOS, volume, applyVolume, isHost]);
